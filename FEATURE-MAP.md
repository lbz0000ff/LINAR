...
> 用途：列出 EchoLily 所有功能的能力边界，供 GUI 开发参考。
...
|------|:---:|:--------:|
| 会话列表 | ✅ | ✅ `list_sessions` |
| 切换会话 | ✅ `/session N` | ✅ `switch_session`+`get_session` |
| 新建会话 | ✅ 自动 | ✅ `new_session` |
...
| 功能 | TUI | 后端支持 |
|------|:---:|:--------:|
| 设置（配置编辑） | ✅ 文件修改 | ✅ `get_config_json`/`save_config` |
| 暗色模式 | ❌（TUI 无） | ❌ 纯前端 |
| MCP 热加载 | ✅ `/reload_mcp` | ✅ |
