...
# ── 导入终端状态管理器 ───────────────────────────────────────
_CLI_DIR = os.path.join(_AGENT_DIR, 'cli')
if _CLI_DIR not in sys.path:
    sys.path.insert(0, _CLI_DIR)
from terminal_state import get_terminal_manager, set_terminal_app, restore_terminal_now
import database
    ...
    def run(self) -> None:
        ...
        async def _main():
            ...
            try:
                ...
                # ── 强制恢复终端状态（双重保险） ─────────────────────────
                get_terminal_manager().restore_terminal(force=True)
