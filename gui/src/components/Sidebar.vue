...
const statusLabel = computed(() => {
  if (props.status === 'connected') return t('sidebar.connected')
  if (props.status === 'reconnecting') return t('sidebar.reconnecting')
  ...
  <aside id="sidebar" ref="sidebarEl" :class="{ collapsed }">
    ...
    <div id="sidebar-header">
      <div id="sidebar-brand">
        <span id="sidebar-title" v-show="!collapsed">LINAR</span>
        <!-- 连接状态呼吸灯 (仅展开模式) -->
      ...
      <div id="sidebar-header-actions" v-show="!collapsed">
        <button id="dark-toggle" @click="emit('darkToggle')" :title="darkMode ? $t('sidebar.lightMode') : $t('sidebar.darkMode')">
          <!-- 太阳 SVG -->
        ...
        <button id="collapse-btn" @click="toggleCollapse" :title="$t('sidebar.collapseTooltip')">
    ...
    <div id="sidebar-body" v-show="!collapsed">
      <input id="search-input" v-model="filter" :placeholder="$t('sidebar.searchPlaceholder')" />
      ...
      <div id="session-list">
        <div v-for="s in filtered" :key="s.id"
          ...
          <template v-else>
            <span class="session-title" @dblclick.stop="startRename(s.id, s.title || t('sidebar.session') + s.id)">
              {{ s.title || $t('sidebar.session') + s.id }}
            </span>
          </template>
    ...
    <div id="sidebar-collapsed" v-show="collapsed">
      ...
      <!-- 展开按钮 -->
      <button class="collapsed-icon" @click="toggleCollapse" :title="$t('sidebar.expandTooltip')">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
      </button>

      <!-- 暗色模式切换 -->
      <button class="collapsed-icon" @click="emit('darkToggle')" :title="darkMode ? $t('sidebar.lightMode') : $t('sidebar.darkMode')">
      ...
      <!-- 新建对话 -->
      <button class="collapsed-icon" @click="emit('new')" :title="$t('sidebar.newSession')">
      ...
      <!-- 设置 (底部) -->
      <button class="collapsed-icon" @click="emit('settings')" :title="$t('sidebar.settings')">
