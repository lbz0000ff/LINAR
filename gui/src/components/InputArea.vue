...
const COMMANDS = computed(() => [
  ...
  { cmd: '/reasoning', desc: t('commands.reasoning') },
  { cmd: '/reset', desc: t('commands.reset') },
  { cmd: '/stop', desc: t('commands.stop') },
  { cmd: '/steer', desc: t('commands.steer'), hasArg: true },
  { cmd: '/btw', desc: t('commands.btw'), hasArg: true },
  { cmd: '/workspace', desc: t('commands.workspace') },
  { cmd: '/workspace-create', desc: t('commands.workspaceCreate'), hasArg: true },
  { cmd: '/workspace-switch', desc: t('commands.workspaceSwitch'), hasArg: true },
  { cmd: '/sessions', desc: t('commands.sessions') },
  { cmd: '/session', desc: t('commands.session'), hasArg: true },
  { cmd: '/exit', desc: t('commands.exit') },
])
...
const allCommands = computed(() => {
  const skillCmds = (props.skills || []).map(s => ({
    cmd: '/' + s.name, desc: s.desc || t('commands.loadSkill') + s.name, isSkill: true,
  }))
  return [...COMMANDS.value, ...skillCmds]
})
  ...
  <div id="input-area" ref="inputAreaEl" class="glass-mac">
    ...
    <div v-if="cmdOpen" id="cmd-panel">
      ...
      <div class="cmd-list">
        ...
        <div v-if="!filteredCommands.length" class="cmd-empty">{{ $t('input.noMatch') }}</div>
      </div>
    </div>
    ...
    <div id="input-toolbar">
      <div id="toolbar-left">
        <button class="tb-btn tb-icon" @click="fileInput.click()" :title="$t('toolbar.attach')">
        ...
        <button class="tb-btn tb-icon" @click="cmdOpen = !cmdOpen" :title="$t('toolbar.commands')">
      ...
      >
        ...
        <span v-else class="stop-text">{{ $t('toolbar.stop') }}</span>
      </button>
    </div>
  </div>
</template>
