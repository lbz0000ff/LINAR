...
const STATUS_CFG = computed(() => ({
  PENDING:    { cls: 'st-pending',    icon: '○', label: t('plan.statusPending') },
  IN_PROGRESS:{ cls: 'st-running',    icon: '◉', label: t('plan.statusInProgress') },
...
<template>
  <div class="pp-section">
    <div class="pp-title">{{ $t('plan.title') }}</div>
    <div v-if="!active && !hasNodes" class="pp-empty">{{ $t('plan.noTasks') }}</div>
    <template v-else>
      ...
      <div class="pp-dag">
        <div v-for="(level, li) in dagLevels" :key="li" class="pp-level">
          <div class="pp-level-label">
            Stage {{ li + 1 }}
            <span v-if="level.length > 1" class="pp-level-badge">{{ $t('plan.parallel', { n: level.length }) }}</span>
          </div>
      ...
      <div v-if="!hasNodes && active" class="pp-empty">{{ $t('plan.waitingNodes') }}</div>
    </template>
  </div>
</template>
